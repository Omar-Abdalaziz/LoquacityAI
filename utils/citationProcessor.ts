...
export const processCitedContent = (
  ...
  for (const citation of processedCitations) {
    ...
    // Create the HTML for the citation button.
    const citationNumber = citation.sourceIndex + 1;
    const safeTitle = citation.sourceTitle.replace(/"/g, '&quot;');
    const ariaLabel = `Source ${citationNumber}: ${safeTitle}`;
    
    const citationButton = ` <sup class="citation"><button class="citation-trigger" data-source-index="${citation.sourceIndex}" title="${safeTitle}" aria-label="${ariaLabel}">${citationNumber}</button></sup>`;
    resultParts.push(citationButton);

...
// FIX: Add FinanceData to the types import to support the new fetchFinanceData function.
import { FinalAnswer, Source, Citation, Personalization, ImageSearchResult, FinanceData, NewsArticle } from '../types';
...
**1. STRUCTURE & EFFICIENCY (CRITICAL):**
- Your response must be well-organized and structured, but also efficient. While maintaining a formal, academic tone, prioritize delivering the core information clearly and without unnecessary verbosity.
- You MUST use the integrated citation tool when you use information from the provided search results.
- **DO NOT** manually add citation numbers like \`[1]\`, \`[2]\`, or \`[1,2]\` in the text. The system will handle displaying citations automatically based on the grounding metadata you provide.
...
// FIX: Implement the missing fetchFinanceData function to provide data for the Finance page.
export const fetchFinanceData = async (): Promise<FinanceData> => {
    const prompt = `Provide a comprehensive financial market overview for today. I need the following information in a single JSON object:
1.  **marketIndices**: A list of 3 major global stock market indices (e.g., S&P 500, NASDAQ, Dow Jones). For each, include: name, current value, change, percentChange, and a boolean 'isPositive'.
2.  **marketAnalysis**: A brief, one-paragraph summary of today's market sentiment and key drivers.
3.  **topMovers**: An object with two lists, 'gainers' and 'losers'. Each list should contain 5 notable stocks. For each stock, include: ticker, name, current price, percentChange, and a boolean 'isPositive'.
4.  **financialNews**: A list of 4 recent, top financial news articles. For each article, include: title, source (e.g., Reuters, Bloomberg), url, a short summary, publishedAt (as an ISO string), and an imageUrl if available.`;

    const systemInstruction = `You are a financial data provider. Your response MUST be a single, valid JSON object that strictly adheres to the requested structure. Do not include any introductory text, markdown, or explanations outside of the JSON object. The entire response must be the raw JSON. Ensure all fields are populated with realistic, current-style data.`;
    ...
    try {
                    ...
                    properties: {
                        ...
                        financialNews: {
                            ...
                            items: {
                                ...
                                required: ["title", "source", "url", "summary", "publishedAt"]
                            }
                        }
                    },
        ...
        if (!data.marketIndices || !data.topMovers || !data.financialNews) {
            throw new Error("Received incomplete financial data from AI.");
        }
